...
spec(call, xc_c(Ctr, c(M,A), R, S)) := XC2 :-
...
spec(ret,  xc_c(Ctr, c(M,A), R, S)):= XC2 :-
    ConfN = ~xrunv5_1(xc_v5(Ctr, c(M,A, []), R, S)),
    ConfN = xc_v5(Ctr1, Conf1, R1, S1),
    Conf1 = c(M1, A1, _),                   
    XC2 = xc_c(Ctr1, c(M1,A1), R1, S1)
...
% V1 rollback
rlb(spec(_,_,_,_,_), xc_c(Ctr, Conf, R, S)) := XC2 :-
...
% V4 rollback
rlb(spec4(_,_,_), xc_c(Ctr, Conf, R, S)) := XC2 :-
    ConfN = ~xrunv4_1(xc41(Ctr, Conf, S)),
    ConfN = xc41(Ctr1, Conf1, S1),
    XC2 = xc_c(Ctr1, Conf1, R, S1).
   
% V5 rollback
rlb(spec5(_,_,_,_,_), xc_c(Ctr, c(M,A), R, S)) := XC2 :-
    ConfN = ~xrunv5_1(xc_v5(Ctr, c(M,A,[]), R, S)),
    ConfN = xc_v5(Ctr1, Conf1, R1, S1),
    Conf1 = c(M1, A1, _),                   
    XC2 = xc_c(Ctr1, c(M1,A1), R1, S1).
...
:- export(xrun_c/4).
xrun_c(xc_c(Ctr, Conf, R, S), Z, Timeout) := Conf :- Timeout =< 0, !,
    trace(timeout).

xrun_c(xc_c(Ctr, Conf, R, S), Z, Timeout) := XC2 :-
     (stop(Conf), S = [] -> XC2 = xc_c(Ctr,Conf, R, S)
        % Note: S=[] (so that spec continue decr otherwise)
	; XC1 = ~xrun_c_1(xc_c(Ctr,Conf, R, S), Z), %Conf = c(_M,A), write( ~p(~pc(A))),
	  ( stop(Conf) -> Timeout1 = Timeout
	  ; Timeout1 is Timeout-1,
	    track_ins(Conf)
	  ),
	  XC2 = ~xrun_c(XC1, Z, Timeout1)
	).
...
xrun_c_1(Conf, Z) := CT :-
    ...
    (enabled(S) -> 
        (member(K, Z) -> CT = ~spec(K, Conf) ; CT = ~nospec(Conf)) % Check if instr is in spec list Z
        ;
           S = [ST | S2], CT = ~rlb(ST, Conf)   ) % In this case S cannot be empty
    .
